...
# Discover queue URL
QUEUE_URL=$(aws sqs get-queue-url --queue-name clawd-bot-tasks-codex --region "$REGION" --query 'QueueUrl' --output text)
log "codex-runner started. Queue: $QUEUE_URL"
...
while true; do
    ...
    if [ -z "$REPO" ]; then
        ...
        send_email "[clawd-bot/codex] $PROJECT: unknown project" \
    ...
    EXIT_CODE=0
    timeout 7200 codex exec "$PROMPT" \
        --full-auto \
        > "$OUTPUT_FILE" 2>&1 || EXIT_CODE=$?

    CODEX_OUTPUT=$(head -c 50000 "$OUTPUT_FILE")
    rm -f "$OUTPUT_FILE"
    ...
    OUTPUT_JSON=$(printf '%s' "$CODEX_OUTPUT" | head -c 50000 | jq -Rs .)
    ...
    # Send notification
    send_email "[clawd-bot/codex] $PROJECT: $STATUS" \
        "Task: $TASK_ID
Project: $PROJECT
Provider: codex
Status: $STATUS
